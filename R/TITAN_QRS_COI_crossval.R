...
## Iterating over installations, we identify de novo indicators based on COI metabarcoding data
...
dim(otusR.COI.p.ra.ab) #2921 SVs, 100 samples
dim(asp) #780
...
for (t in trainOn) {
  ...
  md.trainAll = md.COI.p[included_samples,]
  otus.trainAll = otusR.COI.p.ra.ab[included_samples,]
  taxa.trainAll = asp[included_samples,]
  ...
  for (pf in platforms){
    ...
    write.table(preds,paste("TITAN2_Splines/SplinePreds_COI/",t,".tsv",sep=""),
...
for (t in trainOn) {
  ...
  ## Read COI data
  preds = read.table(paste("TITAN2_Splines/SplinePreds_COI/",t,".tsv",sep=""), 
  ...
  md.trainAll = md.COI.p[included_samples,]
  table(row.names(preds) == row.names(md.trainAll))
  
  
  ## Draw correlation plot
  pdf(paste("img/Splines/",t,"_COI_OTUs_crossval_pred",extra,".pdf",sep=""),width=8,height=5)
  ...
  pdf(paste("img/Splines/",t,"_COI_Taxa_crossval_pred",extra,".pdf",sep=""),width=8,height=5)
  ...
  ## Read 18S data for consensus plot and make consensus pred plots
  preds.18S =  read.table(paste("TITAN2_Splines/SplinePreds_18S/",t,".tsv",sep=""), 
                          sep="\t", header=F, skip=1, row.names=1)
  
  colnames(preds.18S) = c("byOTUs","byTaxa")
  included_samples.18S = !is.na(md.18S.p[,t])
  md.train18S = md.18S.p[included_samples.18S,]
  table(row.names(preds.18S) == row.names(md.train18S))
  row.names(md.train18S) = paste(row.names(md.train18S),"18S",sep=".")
  row.names(preds.18S) = paste(row.names(preds.18S),"18S",sep=".")
  md.train.both = rbind(md.trainAll, md.train18S)
  preds.both = rbind(preds,preds.18S)
  table(row.names(md.train.both) == row.names(preds.both))
  
  pdf(paste("img/Splines/",t,"_18S_COI_consensus_OTUs_crossval",extra,".pdf",sep=""),width=8,height=5)
  plot_ml(data = preds.both$byOTUs[!is.na(preds.both$byOTUs)],
          metadata = md.train.both[!is.na(preds.both$byOTUs),],
          xIndex = t, yIndex=t,
          aggreg = c("Station", "Platform"),
          title = "Spline predicted BI values")
  dev.off()
  
  pdf(paste("img/Splines/",t,"_18S_COI_consensus_Taxa_crossval",extra,".pdf",sep=""),width=8,height=5)
  plot_ml(data = preds.both$byTaxa[!is.na(preds.both$byTaxa)], 
          metadata = md.train.both[!is.na(preds.both$byTaxa),], 
          xIndex = t, yIndex=t,
          aggreg = c("Station", "Platform"),
          title = "Spline predicted BI values")
  dev.off()
