...
## Iterating over installations, we identify de novo indicators based on 18S metabarcoding data
...
for (t in trainOn) {
  ...
  md.trainAll = md.18S.p[included_samples,]
  otus.trainAll = otus.18S.np.p.ra.ab[included_samples,]
  taxa.trainAll = asp_18S_np[included_samples,]
  ...
  for (pf in platforms){
    ...
    write.table(preds,paste("TITAN2_Splines/SplinePreds_18S/",t,".tsv",sep=""),
...
for (t in trainOn) {
  ...
  preds = read.table(paste("TITAN2_Splines/SplinePreds_18S/",t,".tsv",sep=""), 
  ...
  pdf(paste("img/Splines/",t,"_18S_OTUs_crossval_pred",extra,".pdf",sep=""),width=8,height=5)
  ...
  pdf(paste("img/Splines/",t,"_18S_Taxa_crossval_pred",extra,".pdf",sep=""),width=8,height=5)
